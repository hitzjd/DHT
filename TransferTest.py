...
class TorrentApp(object):
    ...
    def __init__(self, metainfo, config, bitstring=None):
    ...
    def start_torrent(self,metainfo,save_incomplete_as,save_as, peerlist=None):
        ...
        try:
            ...
            def create_finished(torrent):
                ...
                if self.torrent.is_initialized():
                    multitorrent.start_torrent(self.torrent.infohash, peerlist) #need to change
                else:
        ...
        try: 
            self.multitorrent = \
                MultiTorrent(self.config, rawserver, data_dir,
                             ...
                             resume_from_torrent_config = False, bitstring=self.bitstring)
                
            self.d.set_torrent_values(metainfo.name, os.path.abspath(saveas),
                                metainfo.total_bytes, len(metainfo.hashes))
            self.start_torrent(self.metainfo, save_incomplete_as, saveas, peerlist)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # allow multi downloads of same torrent
        config['one_download_per_torrent'] = False
        config['bind'] = '202.118.236.124'
        torrentfile = 'data\\files\\dataFile.txt.torrent'

        try:
            metainfo = GetTorrent.get(torrentfile)
            # print metainfo.__dict__
    ...
    peerlist1 = [('202.118.236.124', 6881)]
    bitstring = chr(0b10000000)
    # app = TorrentApp(metainfo, config, bitstring)
    app = TorrentApp(metainfo, config)
    try:
        # config['save_in'] = u'C:\\Users\\Administrator\\Desktop\\DHT\\data\\files\\peer1'
        # app.run()
        config['minport'] = config['minport']+1
        config['save_in'] = u'C:\\Users\\Administrator\\Desktop\\DHT\\data\\files\\peer2'
        app.run(peerlist1)
    except KeyboardInterrupt:
        pass
    except BTFailure, e:
